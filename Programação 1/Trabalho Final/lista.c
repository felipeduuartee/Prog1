...
int lista_insere (lista_t* lst, int elem, int pos) {
	...
	if (!no)
		return -1;
	// no = elemento fornecido
	no->val = elem;

	// lista vazia, novo nó primeiro e ultimo da lista
	if (lst->size == 0) {
		no->next = NULL;
		no->prev = NULL;
		lst->head = no;
	...
	// posicao 0, novo nó se torna cabeça
	else if (pos == 0) {
		no->next = lst->head;
		no->prev = NULL;
		lst->head->prev = no;
	...
	// posicao -1 ou maior que o tamanho, nó se torna ultimo da lista
	...
	// se a posicao estiver no meio da lista
	else {
		...
		for (int i = 0; i < pos - 1; i++) {
			aux = aux->next; // nó anterior à inserção
		}

		no->prev = aux;
		no->next = aux->next;
		aux->next = no;
		aux->next->prev = no; // inserido entre o nó anterior e o proximo
	}
...
int lista_retira (lista_t* lst, int* elem, int pos) {
	...
	// caso onde o elemento a ser removido é o primeiro
	else if ( pos == 0 && lst->size > 1 ) { 
		aux = lst->head;
		lst->head = aux->next;
		lst->head->prev = NULL;
		aux->next = NULL;
	...
	// Caso onde o elemento a ser removido está entre o primeiro e o último
	else if( pos > 0 && pos < lst->size -1) { 
		aux = lst->head;

		for (i = 0; i < pos && i < lst->size - 1; i++) 
			aux = aux->next;

		//desconecta o nodo a ser retirado e conecta o antecessor ao sucessor
